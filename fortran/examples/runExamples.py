...
isABatchSystemUsed = verifyVariableExists("PERFECT_IS_A_BATCH_SYSTEM_USED")
runLargeExamplesStr = verifyVariableExists("PERFECT_RUN_LARGE_EXAMPLES")
commandToSubmitJob = verifyVariableExists("PERFECT_COMMAND_TO_SUBMIT_JOB")
retestStr = verifyVariableExists("PERFECT_RETEST")
...
if retestStr=="yes":
    ...
    print "Error! PERFECT_RETEST must be either 'yes' or 'no'. There is likely an error in the main makefile."
    exit(1)
...
if runLargeExamplesStr=="yes":
    ...
    print "Error! PERFECT_RUN_LARGE_EXAMPLES must be either 'yes' or 'no'.  There is likely an error in the makefile for your system."
    exit(1)
...
try:
    ...
    print "Error! Use of 'make test' for PERFECT requires the utility h5dump, which is usually available whenever the HDF5 library is available. However, an exception was raised when trying to execute h5dump just now."
    raise
...
if isABatchSystemUsed == "yes":
    ...
    print "by trying to read perfectOutput.h5 using h5dump rather than by directly checking the batch queue.  Therefore"
    print "if any of the example jobs crash before opening perfectOutput.h5, or without cleanly writing perfectOutput.h5,"
...
if isABatchSystemUsed == "no":
    for subdirectory in examplesToRun:
        ...
        if not retest:
            try:
                os.remove("perfectOutput.h5")
            except:
                pass
            # If perfectOutput.h5 does not exist, there will be an exception, but we can safely ignore it.

            print "Lanching PERFECT..."
            try:
                # Next we launch PERFECT.
                ...
                print "An error occurred when attempting to launch perfect."
                print "This is likely due to an error in the PERFECT_COMMAND_TO_SUBMIT_JOB parameter in the makefile for your system."
                print "The present value of PERFECT_COMMAND_TO_SUBMIT_JOB is: " + os.environ["PERFECT_COMMAND_TO_SUBMIT_JOB"]
                raise
    ...
    if retest:
        ...
        for subdirectory in examplesToRun:
            ...
            try:
                ...
                print "Unable to submit example "+subdirectory+" for some reason. The problem may be PERFECT_COMMAND_TO_SUBMIT_JOB for your system. Skipping this example."
            else:
    ...
    # Periodically ping each example to see if its perfectOutput.h5 output is readable.
    while keepGoing:
        time.sleep(4)
        print "Checking whether jobs have finished (by trying to read perfectOutput.h5 files.) Press Ctrl-C to quit."
        keepGoing = False
        for subdirectory in examplesSubmitted:
            filename = subdirectory+"/perfectOutput.h5"
            if status[subdirectory]=="completedWithErrors":
                ...
                if result != 0:
                    print "   Example "+subdirectory+" has started but I cannot yet read its perfectOutput.h5."
                    keepGoing = True
                else:
    ...
    print "Error! Environment variable PERFECT_IS_A_BATCH_SYSTEM_USED must be either 'yes' or 'no'.  There is probably a mistake in your system's makefile"
    exit(1)
