...
if exist(geometryFilePath,'file') == 2
  % File exsists, so append
  hdf5write(geometryFilePath, strcat(group,'psiMin'), psiMin, 'WriteMode', 'append')
else
  % Need to create file
  hdf5write(geometryFilePath, strcat(group,'psiMin'), psiMin, 'WriteMode', 'overwrite')
end
