...
#include <petsc/finclude/petscsys.h>
  ...
  contains
    ...
    subroutine computeRosenbluthPotentialResponse(Nx, x, xWeights, Nspecies, mHats, THats, nHats, Zs, NL, Npsi, &
      ...
      integer, intent(in) :: Nx, Nspecies, NL, Npsi
      PetscScalar, dimension(:), intent(in) :: x, xWeights, mHats, Zs
      PetscScalar, dimension(:,:), intent(in) :: THats, nHats
      PetscScalar, dimension(:,:,:,:,:,:), intent(out) :: RosenbluthPotentialTerms
      ! Order of indicies in the Rosenbluth response matrices:
      ! (species_row, species_col, L, x_row, x_col, psi)
      ...
      integer :: i, L, iSpeciesA, iSpeciesB, ix, imode, ipsi
         ...
         do ipsi = 1,Npsi
            do iSpeciesA = 1,Nspecies
               do iSpeciesB = 1,Nspecies
                  speciesFactor = sqrt(THats(iSpeciesA,ipsi)*mHats(iSpeciesB) &
                       / (THats(iSpeciesB,ipsi) * mHats(iSpeciesA)))

                  speciesFactor2 = 3/(2*pi)*nHats(iSpeciesA,ipsi) &
                       * Zs(iSpeciesA)*Zs(iSpeciesA)*Zs(iSpeciesB)*Zs(iSpeciesB) &
                       / (THats(iSpeciesA,ipsi) * sqrt(THats(iSpeciesA,ipsi)*mHats(ispeciesA))) &
                       * THats(iSpeciesB,ipsi)*mHats(iSpeciesA)/(THats(iSpeciesA,ipsi)*mHats(iSpeciesB))
                  ...
                  do i=1,Nx
                     !tempMatrix_combined(i,:) = tempMatrix_combined(i,:) &
                  ...
                  RosenbluthPotentialTerms(iSpeciesA, iSpeciesB, L+1, :, :, ipsi) = matmul(tempMatrix_combined, collocation2modal)
...
!!$                  Rosenbluth_d2Gdxb2(iSpeciesA, iSpeciesB, L+1, :, :) = matmul(tempMatrix_d2Gdxb2, collocation2modal)
                  ...
                  if (verbose) then
                     print *,"For iSpeciesA=",iSpeciesA,",iSpeciesB=",iSpeciesB,",L=",L,",ipsi=",ipsi
                     print *,"RosenbluthPotentialTerms="
                     do ix=1,Nx
                        print *,RosenbluthPotentialTerms(iSpeciesA,iSpeciesB,L+1,ix,:,ipsi)
                     end do
                  end if
               end do
            end do
